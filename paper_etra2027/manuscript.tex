...
While the canthal projection eliminates 1st-order planar head movement, 2nd-order physical phenomena remain: perspective foreshortening as the face rotates, camera distance fluctuations, binocular vergence, and asymmetric eyelid occlusion. OpenGaze captures these phenomena by expanding the canthal offsets into a structured 28-dimensional feature vector $\boldsymbol{\phi} \in \mathbb{R}^{28}$:

\begin{enumerate}[leftmargin=*]
    \item \textbf{Intercept and Base Gaze ($\phi_{0:2}$)}: $\phi_0 = 1.0$, $\phi_1 = g_x$, $\phi_2 = g_y$, representing the unconstrained binocular gaze vectors.
    \item \textbf{Ocular Disparity, EAR, and Normalized Distance ($\phi_{3:5}$)}: $\phi_3 = \text{disparity} = g_{x,L} - g_{x,R}$, capturing depth vergence; $\phi_4 = \text{EAR}$, tracking mean palpebral aperture; $\phi_5 = (\text{faceScale} - \text{faceScale}_{\text{anchor}}) / \text{faceScale}_{\text{anchor}}$, capturing camera distance variations.
    \item \textbf{Relative Head-Pose Anchors ($\phi_{6:8}$)}: Relative yaw, pitch, and roll angles: $\phi_6 = \text{yaw} - \text{yaw}_{\text{anchor}}$, $\phi_7 = \text{pitch} - \text{pitch}_{\text{anchor}}$, and $\phi_8 = \text{roll}$.
    \item \textbf{Decoupled Monocular Kinematics ($\phi_{9:18}$)}: Separate channels for the left eye ($g_{x,L}, g_{y,L}, \text{EAR}_L, \text{width}_L, r_{\text{iris},L}$) and right eye ($g_{x,R}, g_{y,R}, \text{EAR}_R, \text{width}_R, r_{\text{iris},R}$). Decoupling the eyes allows independent weights to compensate for unilateral eyelid drooping or off-axis illumination.
    \item \textbf{Optical Perspective Invariants ($\phi_{19:20}$)}: Dimensionless ratios that isolate 3D perspective foreshortening independent of user distance:
    \begin{equation}
        \Delta W_{\text{eye}} = \frac{\text{width}_L - \text{width}_R}{\text{width}_L + \text{width}_R + \epsilon}, \quad \Delta R_{\text{iris}} = \frac{r_{\text{iris},L} - r_{\text{iris},R}}{r_{\text{iris},L} + r_{\text{iris},R} + \epsilon}
    \end{equation}
    Empirically, $\Delta R_{\text{iris}}$ achieves $r = -0.9375$ correlation with horizontal screen gaze.
    \item \textbf{Planar Tangent Projections ($\phi_{21:22}$)}: $\phi_{21} = \tan(\text{clip}(g_x, -1.2, 1.2))$ and $\phi_{22} = \tan(\text{clip}(g_y, -1.2, 1.2))$, representing the exact trigonometric projection of ocular angle onto planar computer displays ($X_{\text{screen}} = D \cdot \tan(\theta)$).
    \item \textbf{Distance-Scaled Gaze ($\phi_{23:24}$)}: $\phi_{23} = g_x \cdot (\text{faceScale} / \text{faceScale}_{\text{anchor}})$ and $\phi_{24} = g_y \cdot (\text{faceScale} / \text{faceScale}_{\text{anchor}})$, capturing visual angle scaling as viewing distance shifts.
    \item \textbf{Differential Squint ($\phi_{25}$)}: $\phi_{25} = \text{EAR}_L - \text{EAR}_R$, detecting asymmetric eyelid squinting at screen boundaries.
    \item \textbf{Head-Gaze Kinematic Cross-Terms ($\phi_{26:27}$)}: $\phi_{26} = g_x \cdot \text{yaw}$ and $\phi_{27} = g_y \cdot \text{pitch}$, decoupling simultaneous eye-head coordination during visual scanning.
\end{enumerate}
...
We evaluated OpenGaze across $N=4$ independent in-browser calibration sessions comprising 1,693 captured video frames across 80 target fixation clusters on a 1920$\times$1080 display at a natural viewing distance of 55--65\,cm. Participants followed a 20-point serpentine calibration grid covering 90\% of the active viewport.
...
\begin{table*}[t]
\centering
\caption{Empirical comparison of gaze estimation architectures across multi-session in-browser datasets ($N=4$ sessions, 1,693 frames). Evaluated via 20-Fold Grouped Leave-One-Target-Out Cross-Validation.}
...
\midrule
Classical Poly (WebGazer OLS) \cite{papoutsaki2016webgazer} & 211.5 & 399.5 & 34.3 & 0.008 \\
Standard Linear Ridge & 220.2 & 411.6 & 872.0 & 0.005 \\
Standard Polynomial Ridge & 211.6 & 398.3 & $5.4 \times 10^5$ & 0.011 \\
Production Per-Eye Ridge Baseline & 221.0 & 405.3 & 900.1 & 0.008 \\
\textbf{OpenGaze (Proposed 28-Term ElasticNet)} & \textbf{123.1} & \textbf{254.3} & $5.0 \times 10^{10}$ & \textbf{0.828} \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Classical Polynomial (WebGazer Baseline)}: Achieves an average CV RMS of 211.5\,px with severe worst-case 95th-percentile error (399.5\,px).
    \item \textbf{Linear and Per-Eye Ridge}: Standard linear models exhibit high structural bias (220.2\,px). Adding per-eye channels improves stability (221.0\,px) but cannot model non-linear perspective curvature.
    \item \textbf{OpenGaze (Proposed)}: Achieves a Mean CV RMS of \textbf{123.1\,px} (and down to \textbf{110.9\,px} on Dataset 1, and \textbf{111.15\,px} when combined with multivariate outlier rejection), representing a \textbf{41.8\% to 52.7\% error reduction} over WebGazer. 95th-percentile tail error drops dramatically from 399.5\,px to \textbf{254.3\,px}.
    \item \textbf{Computational Efficiency}: Average inference latency is \textbf{0.828\,ms} per frame on a standard single CPU thread, easily sustaining 60\,FPS tracking with over 90\% CPU headroom for host applications.
\end{enumerate}

\subsection{Cross-Session Generalization \& Fast 4-Point Affine Adaptation}
A critical challenge in appearance-based eye tracking is performance decay when transferring a calibrated model across different sessions or seated postures. Evaluating zero-shot transfer across all pairwise session combinations yields an average cross-session transfer error of \textbf{353.6\,px} (a $4.6\times$ degradation over the 76.6\,px intra-session baseline). 

To resolve this without tedious full re-calibration, OpenGaze implements a rapid \textbf{4-Point Affine Recalibration} protocol. By prompting the user to glance at four cardinal screen corner anchors (requiring $<3\,$seconds), we fit a regularized $3 \times 2$ affine transformation:
\begin{equation}
    \begin{bmatrix} \hat{x}_{\text{corr}} \\ \hat{y}_{\text{corr}} \end{bmatrix} = \begin{bmatrix} a_{11} & a_{12} \\ a_{21} & a_{22} \end{bmatrix} \begin{bmatrix} \hat{x} \\ \hat{y} \end{bmatrix} + \begin{bmatrix} t_x \\ t_y \end{bmatrix}
\end{equation}
Table~\ref{tab:recalibration_adaptation} summarizes the adaptation progression. A 1-point center bias correction reduces transfer error by 9.3\% (320.7\,px), while the 4-point affine recalibration collapses transfer error to \textbf{174.9\,px}---achieving a \textbf{50.5\% error reduction (178.8\,px drop)} in under 3 seconds.

\begin{table}[t]
\centering
\caption{Cross-session adaptation progression across independent sessions.}
\label{tab:recalibration_adaptation}
\begin{tabular}{lccc}
\toprule
\textbf{Adaptation Protocol} & \textbf{Transfer Error (px)} $\downarrow$ & \textbf{Drop vs. Zero-Shot} & \textbf{Calibration Time} \\
\midrule
Zero-Shot Transfer (0-Point) & 353.6 & --- & 0.0\,s \\
Center Bias Correction (1-Point) & 320.7 & $-9.3\%$ ($-32.9\,$px) & 0.8\,s \\
Scale + Bias Correction (2-Point) & 318.1 & $-10.1\%$ ($-35.6\,$px) & 1.5\,s \\
\textbf{Cardinal Affine Adaptation (4-Point)} & \textbf{174.9} & \textbf{$-50.5\%$ ($-178.8\,$px)} & \textbf{2.8\,s} \\
\bottomrule
\end{tabular}
\end{table}
